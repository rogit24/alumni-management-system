...
function AlumniSearch() {
  ...
  useEffect(() => {
    ...
    const alumniWithIds = savedAlumni.map(
      (item, index) => ({
        ...item,
        id:
          item.id ||
 ...
 const requestReferral = (alumniData) => {
  ...
  const alreadyRequested = referrals.find(
    (ref) =>
      ref.alumniEmail?.toLowerCase() ===
        alumniData.email?.toLowerCase() &&
      ref.studentEmail?.toLowerCase() ===
